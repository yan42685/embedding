...
for row in data:
    for i in range(columns_count):
        if row[i] not in dic[i]:
            dic[i][row[i]] = 1
        else:
            dic[i][row[i]] = 1 + dic[i][row[i]]

res = []
threshold1 = 5
threshold2 = 5
for row in data:
    if row[0] in dic[0] and dic[0][row[0]] > threshold1 and row[2] in dic[2] and dic[2][row[2]] > threshold2:
        res.append(row)

df5 = pd.DataFrame(res)
df5.columns = ["head", "relation", "tail", "date"]
